...
# note: must be run before compiler_error
  ...
  reason="utmp_x"
elif bzgrep -qE '(parse error|too (many|few) arguments to|argument.*doesn.*prototype|incompatible type for argument|conflicting types for|undeclared \(first use (in |)this function\)|incorrect number of parameters|has incomplete type and cannot be initialized|error: storage size.* isn.t known|(invalid|unknown) argument)' $1; then
...
# XXX MCL "file not recognized: File format not recognized" can be clang
  ...
  reason="checksum"
elif bzgrep -qE "(clang: error: unable to execute command|error: cannot compile this.*yet|error: clang frontend command failed|error:.*ignoring directive for now|error: (invalid|unknown use of) instruction mnemonic|error:.*please report this as a bug)" $1; then
  reason="clang-bug"
