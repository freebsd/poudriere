...
# First check that without pipe_hold, reader() dies.
{
	...
	assert 0 "$?"
	reader_pid="$!"
	assert_true kill -0 "${reader_pid}"
	echo "Blah1" > "${FIFO}"
	assert 0 "$?"
	# Having closed the pipe the child will die.
	assert_true _wait "${reader_pid}"
...
# Now with pipe_hold
{
	...
	assert 0 "$?"
	reader_pid="$!"
	assert_true pipe_hold pipe_hold_pid "${reader_pid}" "${FIFO}"
	assert_true kill -0 "${reader_pid}"
	echo "Blah1" > "${FIFO}"
	assert 0 "$?"
	assert_true kill -0 "${reader_pid}"
	...
	Blah2
	EOF
	kill_job 1 "${reader_pid}"
	ret="$?"
	case "${ret}" in
	143) ret=0 ;;
	esac
	assert 0 "${ret}"
	kill_job 1 "${pipe_hold_pid}"
	ret="$?"
	case "${ret}" in
	143) ret=0 ;;
	esac
