...

{
	...
	awk '$NF == "INT" {print "trap -- '"''"' " $NF; next} {print}' "${ORIGINAL}" > "${EXPECTED_0}"
	n=0
	while trap_ignore_block tmp INT; do
		trap
...

{
	...
	trap
	EXPECTED_0=$(mktemp -ut trap_save)
	awk '$NF == "INT" || $NF == "HUP" {print "trap -- '"''"' " $NF; next} {print}' "${ORIGINAL}" > "${EXPECTED_0}"
	n=0
	while trap_ignore_block tmp INT HUP; do
		trap
		assert_traps "${EXPECTED_0}" "saved 0 traps should match"
		n=$((n + 1))
		trap foo INT
