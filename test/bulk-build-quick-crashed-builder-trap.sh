...
FP_BUILD_PKG_TRAP_PKGNAMES=foo
# Disable crashed build collection as we want to ensure the token is returned.
# If the token is NOT returned (assuming trap catch worked) then the test will
# timeout. trap working is checked with EXPECTED_CRASHED+bulk $? later.
